/* sum_mpi_ptp.c
 * Sum numbers 1..N using point-to-point sends to root (explicit source in receive)
 */
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        for (int src = 1; src < size; ++src) {
            ...
            MPI_Recv(&recvval, 1, MPI_LONG_LONG_INT, src, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            total += recvval;
        }
